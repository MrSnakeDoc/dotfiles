...
map("n", "i", "a", { desc = "Replace i by a" })
map("n", "a", "i", { desc = "Replace a by i" })
...
end, { desc = "Yank file path relative to repo" })

local auto_register = string.byte("a")

map("n", "<leader>yr", function()
  local reg = string.char(auto_register)

  auto_register = auto_register + 1

  if auto_register > string.byte("z") then
    auto_register = string.byte("a")
  end

  vim.notify('Using register "' .. reg)

  vim.api.nvim_feedkeys(vim.api.nvim_replace_termcodes('"' .. reg, true, false, true), "n", false)
end, { desc = "Use next auto register" })
